...
description: Comprehensive list of RTOS commands and how to use them

!!! info "RTOS provides a powerful control interface using commands"   
...
!!! warning
    If you're using RTOS versions earlier current release some of the commands might not work. 
...
Ping<x\> <addr\><a class="cmnd" id="ping"></a>|`<x>` = `0..8` = the number of ICMP packets to send, `0` uses the default (4)<BR>`<addr>` = address to send Ping, either in numerical format `192.168.1.200` or domain name `RTOS.com`<BR><BR>(requires `#define USE_PING`)<BR>Example `Ping4 192.168.1.203`: ```RSL: tele/RTOS_xxx/RESULT = {"Ping":{"192.168.1.203":{"Reachable":true,"Success":4,"Timeout":0,"MinTime":59,"MaxTime":167,"AvgTime":116}}}```
...
!!! note "Information on sensors documented below is transmitted in the RTOS telemetry message"
...
Sensor50<a class="cmnd" id="Sensor50"></a>|[PAJ7620](PAJ7620) gesture sensor<BR>`0` = sensor muted, no readings in RTOS<BR>`1`= gesture mode<BR>`2` = proximity mode<BR>`3` = corner mode<br>`4` = PIN mode<br>`5` = cursor mode
...
Sensor53<a class="cmnd" id="sensor53"></a>|[Smart Meter Interface](Smart-Meter-Interface)<BR>`r` = reset the driver with a new descriptor specified with the RTOS [Scripting](Scripting-Language) language.<BR>`c<x> <value>` = preset counter (x = `1..5`) to `value` when the driver is set to counter mode<BR>`d<x>` = disable data decoding and dump meter (x = `1..5`) data to the Console. This is used to decipher the meter's data format to define the variable encoding in the meter's descriptor.<BR>`d0` = disable data dump mode and revert to decoding mode.
...
Dimmer&#60;x>|**Commands available only when `SetOption37 >= 128`** ([#6819](https://github.com/arendst/RTOS/pull/6819))<br>`<value>` same as in `Dimmer`<br>`Dimmer0 <value>` = set dimming for all channels<BR>`Dimmer1 <value>` = set dimming for RGB channels<BR>`Dimmer2 <value>` = set dimming for white channels
...
SetOption58<a class="cmnd" id="setoption58"></a>|[IR Raw data in JSON payload](https://github.com/arendst/RTOS/issues/2116#issuecomment-440716483)<BR>`0` = disable *(default)*<BR> `1` = enable
...
SetOption61<a class="cmnd" id="setoption61"></a>|Force [local operation](https://github.com/arendst/RTOS/pull/4562#issuecomment-446230001) when [`ButtonTopic`](#buttontopic) or [`SwitchTopic`](#switchtopic) is set.<BR>`0` = disable *(default)*<BR> `1` = enable
...
SetOption63<a class="cmnd" id="setoption63"></a>|Set relay state feedback scan at restart ([#5594](https://github.com/arendst/RTOS/issues/5594), [#5663](https://github.com/arendst/RTOS/issues/5663))<BR>`0` = Scan power state at restart *(default)*<BR> `1` = Disable power state scanning at restart
...
SetOption75<a class="cmnd" id="setoption75"></a>|Set grouptopic behaviour ([#6779](https://github.com/arendst/RTOS/issues/6779))<BR>`0` = GroupTopic using FullTopic replacing %topic% _(default)_<BR>`1` =  GroupTopic is `cmnd/%grouptopic%/` 
SetOption76<a class="cmnd" id="setoption76"></a>|Bootcount incrementing when [DeepSleep](DeepSleep) is enabled ([#6930](https://github.com/arendst/RTOS/issues/6930))<BR>`0` = disable bootcount incrementing _(default)_<BR>`1` = enable bootcount incrementing 
...
SetOption78<a class="cmnd" id="setoption78"></a>|Version check on RTOS upgrade<BR>`0` = enabled _(default)_<BR>`1` = disabled 
...
|See also<BR>[`SetOption29`](Commands#setoption29)  - Set IR received data format<BR>[`SetOption38`](Commands#setoption38)  - Set IR received protocol sensitivity<BR>[`SetOption58`](Commands#setoption58) - [IR Raw data in JSON payload](https://github.com/arendst/RTOS/issues/2116#issuecomment-440716483)
...
|See also<BR>[`SetOption29`](#setoption29)  - Set IR received data format<BR>[`SetOption38`](#setoption38)  - Set IR received protocol sensitivity<BR>[`SetOption58`](#setoption58) - [IR Raw data in JSON payload](https://github.com/arendst/RTOS/issues/2116#issuecomment-440716483)
...
ZbForget<a class="cmnd" id="zbforget"></a>|Used for devices that are unused but still visible in `ZbStatus`<BR>`<device>` = Remove a device from the RTOS flash memory. It does not un-pair the device nor deleting the device information in the CC2530/ZBBridge.
ZbLight<a class="cmnd" id="zblight"></a>|Sets or reads the light type to be emulated in Zigbee Hue Emulation with Alexa.<BR>`<device>,<light_type>` sets the light type using an integer `0..5` corresponding to the number of channels (from one channel (on/off) to 5 channel (RGBCCT) lights)<BR>`<device>,-1` removes the device from Philips Hue emulation<BR>`<device>` displays the current status of the Light (Zigbee2RTOS tracks all changes to the light)
...
HM10Time <a class="cmnd" id="hm10time"></a>|`<n>` = set time time of a **LYWSD02 only** sensor to RTOS UTC time and timezone. `<n>` is the sensor number in order of discovery starting with 0 (topmost sensor in the webUI list).
